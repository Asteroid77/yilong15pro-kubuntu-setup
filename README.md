# 翼龙 15 Pro → Linux（Kubuntu 26.04）迁移/初始化脚本

面向 **翼龙 15 Pro**（笔记本）从 Windows/原系统迁移到 Linux 后的一键初始化脚本，发行版为 **Kubuntu 26.04**，以系统默认安装作为基准环境进行补齐与配置。
...
| 分区 | 内容 | 备注（脚本额外动作） |
| --- | --- | --- |
| 软件源/镜像 | APT / Docker / NPM / Maven / Go / GitHub 镜像 | 自动应用到对应工具；GitHub 下载支持镜像轮询 |
| 驱动/硬件 | 安装 NVIDIA open driver 580 | 默认安装 `nvidia-driver-580-open`；不再写入额外 GRUB/DRM 兼容参数 |
| 基础依赖 | 常用 CLI、构建工具、输入法、媒体/系统工具等 | 以 Kubuntu 26.04 默认系统为基准补齐 |
...
说明：`--no-interactive` 仍按全量安装执行。终端统一使用 WezTerm；若只需要 `Atuin + glow + Streamdown`，建议使用默认交互模式并取消勾选 `wezterm_tmux`。
...
该入口只启用 `wezterm_tmux` 路线：安装/更新 WezTerm、`tmux`、`tmux-resurrect`、`tmux-continuum`，并写入 WezTerm 专用 `.wezterm.lua`、`~/.config/tmux/wezterm.conf` 和 `wezterm-tmux-tab` helper。它不写入 `~/.tmux.conf`，不读取全局 `~/.local/share/tmux/resurrect`。
...
| 类别 | 动作 | 触发条件/备注 |
| --- | --- | --- |
| NVIDIA | `sudo apt install -y nvidia-driver-580-open` | 使用 Ubuntu 包管理安装 open driver 580 |

## 基础依赖

以 **Kubuntu 26.04 默认系统**为起点，脚本补齐以下基础包（参考： `scripts/steps/01_core.sh`）。
...
| 系统/多媒体 | `btop` `okular` `wireshark` `calibre` `ffmpegthumbs` |
...
| --- | --- | --- | --- |
| `terminal_tools` | 通用 shell 增强 | `Atuin + glow + Streamdown` | 注入 `mdv` / `mdstream` / `mdrun` / `refreshapps` 与 `atuin init` |
| `wezterm_tmux` | 使用 WezTerm GUI tabs 恢复终端工作区 | WezTerm + 独立 tmux socket/config/resurrect state | 安装 WezTerm，写入 `~/.wezterm.lua`、`~/.config/tmux/wezterm.conf`、`wezterm-tmux-tab`；一个 WezTerm GUI tab 对应一个业务 tmux session |

说明：
- 交互勾选里 `terminal_tools` 默认开启，`wezterm_tmux` 默认关闭。
- `wezterm_tmux` 是唯一终端恢复路线，不读取 `~/.tmux.conf` 和全局 `~/.local/share/tmux/resurrect`。它固定使用 socket `~/.local/state/tmux-wezterm/main.sock`、配置 `~/.config/tmux/wezterm.conf`、保存目录 `~/.local/state/wezterm-tmux/resurrect`。恢复事实源是业务 tmux sessions；一个 WezTerm GUI tab 对应一个业务 session，GUI tab 标题来自 tmux session name，tab 内部可继续使用 tmux windows/panes。`Ctrl+Shift+T` 新建一个业务 session 和对应 GUI tab，`Ctrl+Shift+R` 重命名当前业务 session。保存快照时会过滤 `wezterm-tech-*`、旧 `wezterm-tab-*`、`meteor-tab-*` 和旧 `main` 状态。关闭并重开 WezTerm 时，`gui-startup` 会按业务 sessions 重建 GUI tabs。
...
| 终端增强 | `terminal_tools`（Atuin + glow + Streamdown） | GitHub Release `.deb` + `pipx` + 官方安装脚本 | 写入 `~/.config/atuin/config.toml`，向 `~/.zshrc` / `~/.bashrc` 注入 `mdv` / `mdstream` / `mdrun` / `refreshapps` |
| 终端恢复 | `wezterm_tmux`（可选） | GitHub Release `.deb` + GitHub clone + 配置模板 | 安装 WezTerm、`tmux`、`tmux-resurrect`、`tmux-continuum`；写入 WezTerm 专用 `.wezterm.lua`、专用 tmux config 和 `wezterm-tmux-tab` helper |
...
| Python 工具 | uv | 官方安装脚本 | 安装 `uv` Python package manager |
...
脚本已内置日常容器服务部署：会生成 `~/docker-settings/docker-compose-daily.yml`，并按勾选项创建 `~/thunder/*` 目录。
